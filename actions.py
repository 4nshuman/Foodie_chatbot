...
class ActionSendEmail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        email_subject_line = "You requested some " + cuisine.capitalize() + " restaurants in " + str(
            location).capitalize()
        email_message_content = "Hello " + to_email.split('@')[0] + ", Here are the top " + str(
            len(restaurant_email_list)) + cuisine.capitalize() + " restaurants in " + str(
            location).capitalize() + "." + "\n \n"
        for restaurant in restaurant_email_list:
            email_message_content = email_message_content + restaurant['restaurant']['name'] + " in " + \
